...
<title>{{websiteTitle('Sales List')}}</title>
@endsection @push('css')
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
         ...
         <div class="dropdown">
            ...
            @endisset
             ...
             <a href="{{route('admin.sales')}}" class="btn-custom yellow">
            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    <ul class="statuslist p-0">
                        <li><a href="{{route('admin.sales')}}">All ({{$totals->total}})</a></li>
                        <li><a href="{{route('admin.sales',['status'=>'pending'])}}">Pending ({{$totals->pending}})</a></li>
                        <li><a href="{{route('admin.sales',['status'=>'confirmed'])}}">Confirmed ({{$totals->confirmed}})</a></li>
                        <li><a href="{{route('admin.sales',['status'=>'completed'])}}">Completed ({{$totals->completed}})</a></li>
                        <li><a href="{{route('admin.sales',['status'=>'cancelled'])}}">Cancelled ({{$totals->cancelled}})</a></li>
                        <li><a href="{{route('admin.sales',['status'=>'trash'])}}">Trash ({{$totals->trash}})</a></li>
                    </ul>
                </div>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th style="min-width: 130px;">Paid</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="{{route('admin.salesAction',['view',$invoice->id])}}" target="_blank">{{$invoice->invoice}}</a>
                            </td>
                            <td>@if($invoice->company) {{$invoice->company->factory_name}} @endif</td>
                            <td>{{$invoice->items()->count()}} Items
                            ...
                            <td>{{$invoice->currency}} {{$invoice->paid_amount}}</td>
                            <td>{{$invoice->currency}} {{$invoice->due_amount}}
                                
                                <a href="{{route('admin.billCollectionAction',['edit',$invoice->id])}}" style="background: #ff108c;font-size: 12px;color: white;border-radius: 5px;display: inline-block;width: 25px;text-align: center;"><i class="bx bx-plus"></i></a>
                            ...
                            <td>
                                ...
                                <a href="{{route('admin.salesAction',['view',$invoice->id])}}" class="btn-custom success"><i class="bx bx-show"></i></a>
                                @endisset
                                @else
                                @isset(json_decode(Auth::user()->permission->permission, true)['sales']['add'])
                                <a href="{{route('admin.salesAction',['edit',$invoice->id])}}" class="btn-custom"><i class="bx bx-edit"></i></a>
                                @endisset
                                @endif
                                @isset(json_decode(Auth::user()->permission->permission, true)['sales']['delete'])
                                <a href="{{route('admin.salesAction',['delete',$invoice->id])}}" onclick="return confirm('Are You Want To Delete?')" class="btn-custom danger"><i class="bx bx-trash"></i></a>
                                @endisset
                            </td>
                        </tr>
                        @endforeach
