...
<div class="breadcrumb-area">
    ...
    <ol class="breadcrumb">
        ...
        <li class="item"><a href="{{route('admin.sales')}}">Invoices Edit</a></li>
    ...
    <div class="card mb-30">
        <div class="card-header d-flex justify-content-between align-items-center">
             ...
             <div class="dropdown">
                 ...
                 <a href="{{route('admin.salesAction',['view',$invoice->id])}}" target="_blank" class="btn-custom primary">
        ...
        <div class="card-body">
            ...
            <form action="{{route('admin.salesAction',['update',$invoice->id])}}">
            @csrf
            <div class="cardItems">
                @include(adminTheme().'sales.includes.orderItems')
            </div>
	 ...
	 <div class="modal-content">
	   <div class="modal-body">
            ...
            <div class="input-group">
                <input type="text" class="form-control {{$errors->has('name')?'error':''}} SearchQuery" data-type="company" data-url="{{route('admin.salesAction',['search-company',$invoice->id])}}" placeholder="Search name.." required="">
			    <div class="input-group-append">
                    <span class="input-group-text"><i class='bx bx-search'></i></span>
                </div>
			</div>
			<div class="customeSearch searchlist" style="height:200px;overflow:auto;">
		        @include(adminTheme().'sales.includes.searchCompany')
		    </div>
	   </div>
	 </div>
   </div>
...
<script>
    $(document).ready(function() {
        ...
        function updateTotalSummary(){
            ...
            var paidAmount = parseFloat($('.totalPaid').data('amount'));
            if (isNaN(paidAmount)) {
                paidAmount = 0;
            }
        ...
        $(document).on('keyup change', '.updateItem', function () {
            var url = $(this).data('url');
        ...
        $(document).on('click','.removeInstallment',function(){
            var url =$(this).data('url');
            var name =$(this).data('name');
            var data =$(this).val();
        ...
        $(document).on('click', '.emiUpdate', function() {
            // Get values from the form fields
            var dueDate = $('.emiStartDate').val();
            var amount = $('.emiAmount').val();
            var installmentTime = $('.emiTime').val();
        
            // Perform field validation
            if (dueDate === '' || amount === '' || installmentTime === '') {
            ...
            $.ajax({
                url: url, // The route where the data will be sent
                type: 'POST',
                data: data,
                success : function(data){
                    if(data.view){
