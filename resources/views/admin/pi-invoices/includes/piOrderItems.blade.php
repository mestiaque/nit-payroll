        ...
        <div class="row">
            <div class="col-md-4" style="padding:10px;">
                <div class="searchGrid">
                    <input type="text" class="form-control form-control-sm SearchQuery" data-type="goods" data-url="{{route('admin.piInvoicesAction',['search-goods',$invoice->id])}}"  placeholder="Search Item of Goods">
                    <div class="itemSearch searchlist" style="height:200px;overflow:auto;">
                        @include(adminTheme().'pi-invoices.includes.searchGoods',['services'=>App\Models\Post::latest()->where('type',3)->where('status','active')->limit(10)->get()])
        ...
        <div class="table-responsive" style="min-height: 200px;">
            <table class="table table-bordered invoiceTable">
                <tr>
                    ...
                    <th style="width: 120px;min-width: 120px;">Delivery Date</th>
                    <th style="padding: 2px;">
                        <span class="btn-custom success addItem" data-url="{{route('admin.piInvoicesAction',['add-item',$invoice->id])}}" ><i class="bx bx-plus"></i></span>
                    </th>
                </tr>
                ...
                <tr>
                    ...
                    <td style="padding: 2px;">
                        <textarea type="text" class="form-control form-control-sm updateItem" data-name="description" data-url="{{route('admin.piInvoicesAction',['update-item',$invoice->id,'item_id'=>$item->id])}}" placeholder="Description">{{$item->description}}</textarea>
                    </td>
                    <td style="padding: 2px;">
                        <input type="number" step="any" class="form-control form-control-sm updateItem calculate quantity_{{$item->id}}" data-id="{{$item->id}}" data-name="quantity" data-url="{{route('admin.piInvoicesAction',['update-item',$invoice->id,'item_id'=>$item->id])}}" value="{{$item->quantity?:''}}" placeholder="Quantity">
                    </td>
                    <td style="padding: 2px;">
                        <select class="form-control form-control-sm updateItem" data-name="unit" data-url="{{route('admin.piInvoicesAction',['update-item',$invoice->id,'item_id'=>$item->id])}}" >
                        ...
                        <!--<input type="text"  class="form-control form-control-sm updateItem" data-name="unit" data-url="{{route('admin.piInvoicesAction',['update-item',$invoice->id,'item_id'=>$item->id])}}" value="{{$item->unit}}" placeholder="Unit">-->
                    </td>
                    <td style="padding: 2px;">
                        <input type="number" step="any" class="form-control form-control-sm updateItem calculate price_{{$item->id}}" data-id="{{$item->id}}" data-name="price" data-url="{{route('admin.piInvoicesAction',['update-item',$invoice->id,'item_id'=>$item->id])}}" value="{{$item->price > 0?$item->price:''}}" placeholder="Price">
                    </td>
                    <td>
                        <span class="priceTotal priceTotal_{{$item->id}}">{{priceFormat($item->final_price)}}</span>
                    </td>
                    <td style="padding: 2px;">
                        <input type="date" class="form-control form-control-sm updateItem" data-name="delivered_at" data-url="{{route('admin.piInvoicesAction',['update-item',$invoice->id,'item_id'=>$item->id])}}" value="{{$item->delivered_at?Carbon\Carbon::parse($item->delivered_at)->format('Y-m-d'):''}}" >
                    </td>
                    <td style="padding:2px;">
                        <span class="btn-custom danger removeItem" data-url="{{route('admin.piInvoicesAction',['remove-item',$invoice->id,'item_id'=>$item->id])}}"><i class="bx bx-trash"></i></span>
                    </td>
                </tr>
                @endforeach
                @else
