@extends(adminTheme().'layouts.app') @section('title')
<title>{{websiteTitle('Reports')}}</title>
@endsection @push('css')
<link rel="stylesheet" type="text/css" href="{{asset('public/app-assets/vendors/css/tables/datatable/datatables.min.css')}}" />
<link rel="stylesheet" type="text/css" href="{{asset('public/app-assets/vendors/css/tables/extensions/buttons.dataTables.min.css')}}" />
<link rel="stylesheet" type="text/css" href="{{asset('public/app-assets/vendors/css/tables/datatable/buttons.bootstrap4.min.css')}}" />
<style type="text/css">
    .summery .info {
        width: 200px;
        display: inline-block;
        border: 1px solid #dfdbdb;
        margin: 5px;
        padding: 5px 15px;
        border-radius: 5px;
        background: #f8f8fa;
    }
    
    .summery .info h3 {
        font-size: 18px;
        margin: 0;
    }
    
    .summery .info span {
        font-size: 26px;
        font-weight: bold;
    }
    
    .dropdown-toggle::after{
        display:none;
    }
    .table tr th{
        padding:5px;
        background: #f8f8fa;
    }
    
    .table tr td{
        padding:5px;
    }
    
    @media only screen and (max-width: 678px) {

        .summery .info {
            width: 100%;
        }
    }
</style>
@endpush @section('contents')


<section class="flex-grow-1">
    @include(adminTheme().'alerts')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h3>Reports</h3>
            <div class="dropdown">
             <a href="{{route('admin.reports')}}" class="btn-custom yellow">
                 <i class="bx bx-rotate-left"></i>
             </a>
         </div>
        </div>
        <div class="card-content">
            <div class="card-body">
                <form action="{{route('admin.reports')}}">
                   <div class="row">
                        <div class="col-md-2">
                            <select class="form-control" name="concern">
                                <option value="" >All Sister Concern</option>
                                <option value="MG Machineries Corporation" {{request()->concern=='MG Machineries Corporation'?'selected':''}}  >MG Machineries Corporation (MMC)</option>
                                <option value="Embroidery Machine Corporation" {{request()->concern=='Embroidery Machine Corporation'?'selected':''}} >Embroidery Machine Corporation (EMC)</option>
                                <option value="MG Training Centre Institute" {{request()->concern=='MG Training Centre Institute'?'selected':''}} >MG Training Centre Institute (MTCI)</option>
                             </select>
                        </div>
                        <div class="col-md-2">
                            <select class="form-control" name="employee_id">
                                <option value="">All Employee</option>
                                @foreach(App\Models\User::where('admin',true)->where('status',1)->get() as $data)
                                <option value="{{$data->id}}" {{$data->id==request()->employee_id?'selected':''}}>{{$data->name}}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="col-md-6">
                           <div class="input-group">
                               <div class="dropdown">
                                    <button class="dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="border: none;background: #ebebec;height: 100%;">
                                        <i class="bx bx-dots-vertical-rounded"></i>
                                    </button>
                                    <div class="dropdown-menu" style="">
                                        <a class="dropdown-item d-flex align-items-center" href="{{route('admin.reports',['startDate'=>Carbon\Carbon::now()->format('Y-m-d'),'endDate'=>Carbon\Carbon::now()->format('Y-m-d')])}}">
                                             Today
                                        </a>
                                        <a class="dropdown-item d-flex align-items-center" href="{{route('admin.reports',['startDate'=>Carbon\Carbon::now()->subDays(7)->format('Y-m-d'),'endDate'=>Carbon\Carbon::now()->format('Y-m-d')])}}">
                                            Last 7 Days
                                        </a>
                                        <a class="dropdown-item d-flex align-items-center" href="{{route('admin.reports',['startDate'=>Carbon\Carbon::now()->subDays(30)->format('Y-m-d'),'endDate'=>Carbon\Carbon::now()->format('Y-m-d')])}}">
                                            Last 30 Days
                                        </a>
                                        <a class="dropdown-item d-flex align-items-center" href="{{route('admin.reports',['startDate'=>Carbon\Carbon::now()->startOfYear()->format('Y-m-d'),'endDate'=>Carbon\Carbon::now()->format('Y-m-d')])}}">
                                            This Years
                                        </a>
                                    </div>
                                </div>
                                <input type="date" name="startDate" value="{{$startDate->format('Y-m-d')}}" class="form-control {{$errors->has('startDate')?'error':''}}" />
                                <input type="date" name="endDate" value="{{$endDate->format('Y-m-d')}}" class="form-control {{$errors->has('endDate')?'error':''}}" />
                                <button type="submit" class="btn btn-success rounded-0">Search</button>
                            </div>
                       </div>
                   </div>
                </form>
                <hr>
                
                <div class="summery">
                    <div class="info">
                        <h3>Leads</h3>
                        <span>{{$summeryReport['Leads']}}</span>
                    </div>
                    <div class="info">
                        <h3>Customer</h3>
                        <span>{{$summeryReport['Companies']}}</span>
                    </div>
                    <div class="info">
                        <h3>Meating</h3>
                        <span>{{$summeryReport['Meeting']}}</span>
                    </div>
                    <div class="info">
                        <h3>Visits</h3>
                        <span>{{$summeryReport['Visits']}}</span>
                    </div>
                    <div class="info">
                        <h3>Sales</h3>
                        <span>{{number_format($summeryReport['Sales'])}}</span>
                    </div>
                    <div class="info">
                        <h3>Sales Due</h3>
                        <span>{{number_format($summeryReport['SalesDue'])}}</span>
                    </div>
                    <div class="info">
                        <h3>Sales Paid</h3>
                        <span>{{number_format($summeryReport['SalesPaid'])}}</span>
                    </div>
                </div>
                
                @if(request()->employee_id)
                    @php 
                        $us =App\Models\User::find(request()->employee_id);
                    @endphp
                    @if($us)
                        <div class="text-center my-2">
                           <h3>
                           {{$us->name}} 
                           </h3>
                           @if($us->permission)
                           <span style="color: #0829e5;font-weight: bold;">
                               {{$us->permission->name}}
                           </span>
                           @endif
                           <p style="margin: 0;font-size: 20px;color: #cec6c6;">
                               Employee Report
                           </p>
                        </div>
                    @endif
                @endif
                
                @if($summeryReport['Leads'] > 0)
                <div class="my-2">
                    <h4>Leads</h4>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <tr>
                                <th>SL</th>
                                <th>Company name</th>
                                <th>Owner name</th>
                                <th>Mobile</th>
                                <th>Address</th>
                                <th>Cus. Status</th>
                                <th>Employee</th>
                                <th>Date</th>
                                <th>Sister Concern</th>
                            </tr>
                            @foreach($leads as $l=>$lead)
                            <tr>
                                <td>{{$l+1}}</td>
                                <td>{{$lead->factory_name}} </a></td>
                                <td><a href="{{route('admin.leadsAction',['view',$lead->id])}}" target="_blank" > {{$lead->name}}</a></td>
                                <td>{{$lead->mobile}}</td>
                                <td>{{$lead->fullAddress()}}</td>
                                 <td>
                                    @if($lead->customer_status=='Not Potential')
                                    <span class="badge" style="background: #9baaff;font-size: 14px;color: white;" >Not Potential</span>
                                    @elseif($lead->customer_status=='Potential')
                                    <span class="badge" style="background: #5970f3;font-size: 14px;color: white;" >Potential</span>
                                    @elseif($lead->customer_status=='Very Potential')
                                    <span class="badge" style="background: #0829e5;font-size: 14px;color: white;" >Very Potential</span>
                                    @endif
                                </td>
                                <td>
                                    @if($lead->assineeUser)
                                    <a href="{{route('admin.usersCustomerAction',['view',$lead->assineeUser->id])}}" target="_blank" >{{$lead->assineeUser->name}}</a>
                                    @endif    
                                </td>
                                <td>{{$lead->created_at->format('d-m-Y')}}</td>
                                <td>{{$lead->concern}}</td>
                            </tr>
                            @endforeach
                            
                        </table>
                    </div>
                </div>
                @endif
                
                @if($summeryReport['Companies'] > 0)
                <div class="my-2">
                    <h4>Customers</h4>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <tr>
                                <th>SL</th>
                                <th>Deed / CT Serial</th>
                                <th>Company name</th>
                                <th>Customer Name</th>
                                <th>Mobile</th>
                                <th>Address</th>
                                <th>Created By</th>
                                <th>Date</th>
                                <th>Sister Concern</th>
                            </tr>
                            @foreach($customers as $c=>$customer)
                            <tr>
                                <td>{{$c+1}}</td>
                                <td>{{$customer->deed_serial}}</td>
                                <td>{{$customer->factory_name}} </a></td>
                                <td><a href="{{route('admin.companiesAction',['view',$customer->id])}}" target="_blank" > {{$customer->owner_name}}</a></td>
                                <td>{{$customer->owner_mobile}}</td>
                                <td>{{$customer->fullAddress()}}</td>
                                <td>
                                    @if($customer->user)
                                    <a href="{{route('admin.usersCustomerAction',['view',$customer->user->id])}}" target="_blank" >{{$customer->user->name}}</a>
                                    @endif    
                                </td>
                                <td>{{$customer->created_at->format('d-m-Y')}}</td>
                                <td>{{$customer->concern}}</td>
                            </tr>
                            @endforeach
                            
                        </table>
                    </div>
                </div>
                @endif
                
                @if($summeryReport['Meeting'] > 0)
                <div class="my-2">
                    <h4>Meeting</h4>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <tr>
                                <th>SL</th>
                                <th>Meeting Title</th>
                                <th>Participants</th>
                                <th>Date & Time</th>
                                <th>Type</th>
                                <th>Host</th>
                            </tr>
                            @foreach($meetings as $c=>$meeting)
                            <tr>
                                <td>{{$c+1}}</td>
                                <td>{{$meeting->name}}</td>
                                <td>
                                    @foreach($meeting->participantsUsers()->get() as $user)
                                    @if($meeting->type==1)
                                    <span>{{$user->name}} - {{$user->email}}</span>
                                    @else
                                    <span>{{$user->factory_name}} - {{$user->owner_name}}</span>
                                    @endif
                                    @endforeach
                                </td>
                                <td>{{$meeting->created_at->format('d-m-Y h:i A')}}</td>
                                <td>{{ucfirst($meeting->meeting_type)}}</td>
                                <td>{{$meeting->hostUser?$meeting->hostUser->name:'Not Found'}}</td>
                            </tr>
                            @endforeach
                            
                        </table>
                    </div>
                </div>
                @endif
                
                @if($summeryReport['Visits'] > 0)
                <div class="my-2">
                    <h4>Visits</h4>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <tr>
                                <th>SL</th>
                                <th>Visit Date</th>
                                <th>Location</th>
                                <th>Company/Lead</th>
                                <th>Description</th>
                                <th>Visit By</th>
                                <th>Status</th>
                            </tr>
                            @foreach($visits as $c=>$visit)
                            <tr>
                                <td>{{$c+1}}</td>
                                <td>{{$visit->visit_date?Carbon\Carbon::parse($visit->visit_date)->format('d-m-Y'):''}}</td>
                                <td>{{$visit->location}}</td>
                                <td>
                                    @if($visit->type==1)
                                    <span>{{$visit->company?$visit->company->name:''}}</span>
                                    @else
                                    {{$visit->company?$visit->company->factory_name:''}}
                                    @endif
                                </td>
                                <td>{{$visit->description}}</td>
                                <td>{{$visit->assinee?$visit->assinee->name:''}}</td>
                                <td>
                                    @if($visit->status=='Not Potential')
                                    <span class="badge" style="background: #9baaff;font-size: 14px;color: white;" >Not Potential</span>
                                    @elseif($visit->status=='Potential')
                                    <span class="badge" style="background: #5970f3;font-size: 14px;color: white;" >Potential</span>
                                    @elseif($visit->status=='Very Potential')
                                    <span class="badge" style="background: #0829e5;font-size: 14px;color: white;" >Very Potential</span>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
                            
                        </table>
                    </div>
                </div>
                @endif
                
                @if($summeryReport['Sales'] > 0)
                <div class="my-2">
                    <h4>Sales</h4>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <tr>
                                <th>SL</th>
                                <th>Invoice</th>
                                <th>Company</th>
                                <th>Items</th>
                                <th>Bill Amount</th>
                                <th>Paid</th>
                                <th>Due</th>
                                <th>Date</th>
                                <th>Sale By</th>
                            </tr>
                            @foreach($sales as $c=>$sale)
                            <tr>
                                <td>{{$c+1}}</td>
                                <td>{{$sale->invoice}}</td>
                                <td>@if($sale->company) {{$sale->company->factory_name}} @endif</td>
                                <td>
                                    @foreach($sale->items as $item)
                                        {{ $item->description }}
                                    @endforeach
                                    
                                </td>
                                <td>
                                    {{number_format($sale->grand_total)}}
                                    @if($sale->emi_status)
                                    <span class="badge" style="background: #ff108c;font-size: 12px;color: white;">EMI Sale</span>
                                    @endif
                                </td>
                                <td>{{number_format($sale->paid_amount)}}</td>
                                <td>{{number_format($sale->due_amount)}}</td>
                                <td>{{$sale->created_at->format('d.m.Y')}}</td>
                                <td>{{$sale->saleBy?$sale->saleBy->name:''}}</td>
                            </tr>
                            @endforeach
                            
                        </table>
                    </div>
                </div>
                @endif
                
            </div>
        </div>
    </div>
</section>




@endsection 

@push('js') 

<!-- BEGIN: Page Vendor JS-->
     <script src="{{asset('public/app-assets/vendors/js/tables/datatable/datatables.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/datatable/dataTables.buttons.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/datatable/buttons.bootstrap4.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/jszip.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/pdfmake.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/vfs_fonts.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/buttons.html5.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/buttons.print.min.js')}}"></script>
     <script src="{{asset('public/app-assets/vendors/js/tables/buttons.colVis.min.js')}}"></script>
     <!-- END: Page Vendor JS-->
     


<script type="text/javascript">
    $(document).ready(function () {
        $(".dataex-visibility-print").DataTable({ dom: "Bfrtip", buttons: ["print"] }),
            $(".dataex-visibility-message").DataTable({ dom: "Bfrtip", buttons: [{ extend: "print", message: "This print was produced using the Print button for DataTables" }] }),
            $(".dataex-visibility-selector").DataTable({ dom: "Bfrtip", buttons: [{ extend: "print", exportOptions: { columns: ":visible" } }, "colvis"], columnDefs: [{ targets: -1, visible: !1 }] }),
            $(".dataex-visibility-rowselector").DataTable({
                dom: "Bfrtip",
                buttons: [
                    { extend: "print", text: "Print all" },
                    { extend: "print", text: "Print selected", exportOptions: { modifier: { selected: !0 } } },
                ],
                select: !0,
            }),
            $(".dataex-visibility-disable").DataTable({ dom: "Bfrtip", buttons: [{ extend: "print", autoPrint: !1 }] }),
            $(".dataex-visibility-customisation").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        extend: "print",
                        customize: function (t) {
                            $(t.document.body).css("font-size", "10pt").prepend('<img src="http://datatables.net/media/images/logo-fade.png" style="position:absolute; top:0; left:0;" />'),
                                $(t.document.body).find("table").addClass("compact").css("font-size", "inherit");
                        },
                    },
                ],
            });
    });



    $(document).ready(function () {
        $(".dataex-html5-export").DataTable({ dom: "Bfrtip", buttons: ["copyHtml5", "excelHtml5", "csvHtml5", "pdfHtml5","print"] }),
        $(".dataex-visibility-print").DataTable({ dom: "Bfrtip", buttons: ["print"] }),
            $(".dataex-html5-separated").DataTable({ dom: "Bfrtip", buttons: ["copyHtml5", { extend: "csvHtml5", fieldSeparator: "\t", extension: ".tsv" }] }),
            $(".dataex-html5-selectors").DataTable({
                dom: "Bfrtip",
                buttons: [{ extend: "copyHtml5", exportOptions: { columns: [0, ":visible"] } }, { extend: "excelHtml5", exportOptions: { columns: ":visible" } }, { extend: "pdfHtml5", exportOptions: { columns: [0, 1, 2, 5] } }, "colvis"],
            }),
            $(".dataex-html5-background").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        extend: "excelHtml5",
                        customize: function (t) {
                            var e = t.xl.worksheets["sheet1.xml"];
                            $('row c[r^="F"]', e).each(function () {
                                1 * $("is t", this).text().replace(/[^\d]/g, "") >= 5e5 && $(this).attr("s", "20");
                            });
                        },
                    },
                ],
            }),
            $(".dataex-html5-message").DataTable({ dom: "Bfrtip", buttons: [{ extend: "pdfHtml5", message: "PDF created by PDFMake with Buttons for DataTables." }] }),
            $(".dataex-html5-orientation").DataTable({ dom: "Bfrtip", buttons: [{ extend: "pdfHtml5", orientation: "landscape", pageSize: "LEGAL" }] }),
            $(".dataex-html5-image").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        extend: "pdfHtml5",
                        customize: function (t) {
                            t.content.splice(1, 0, {
                                margin: [0, 0, 0, 12],
                                alignment: "center",
                                image:
                                    "data:image/png;base64,",
                            });
                        },
                    },
                ],
            }),
            $(".dataex-html5-window").DataTable({ dom: "Bfrtip", buttons: [{ extend: "pdfHtml5", download: "open" }] }),
            $(".dataex-html5-json").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        text: "JSON",
                        action: function (t, e, a, A) {
                            var o = e.buttons.exportData();
                            $.fn.dataTable.fileSave(new Blob([JSON.stringify(o)]), "Export.json");
                        },
                    },
                ],
            });
    });

</script>

@endpush