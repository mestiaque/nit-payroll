<table class="table table-bordered mcTable">
    <tr>
        ...
        <th style="padding:2px;text-align:center;width: 50px;min-width: 50px;"><span class="btn btn-sm btn-info addPerson" data-type="{{$type}}"  data-url="{{route('admin.companiesAction',['add-person',$company->id])}}"><i class="bx bx-plus"></i></span></th>
    </tr>
    @foreach($company->persons()->where('type',$type)->get() as $i=>$person)
    <tr>
        ...
        <td style="padding:5px;text-align: center;">
            <span class="text-white bg-danger p-1 removePerson" data-type="{{$type}}" data-url="{{route('admin.companiesAction',['remove-person',$company->id,'person_id'=>$person->id])}}" style="border-radius: 3px;cursor: pointer;"><i class="bx bx-trash"></i></span>
        </td>
    </tr>
    @endforeach
    @if($company->persons()->where('type',$type)->count()==0)
    <tr>
