@extends(adminTheme().'layouts.app') 
@section('title')
@if($action=='pm-list')
<title>{{websiteTitle('Company PM List')}}</title>
@elseif($action=='keyperson-list')
<title>{{websiteTitle('Company Key Person List')}}</title>
@elseif($action=='partner-list')
<title>{{websiteTitle('Company Partner List')}}</title>
@elseif($action=='operator-list')
<title>{{websiteTitle('Company Operator List')}}</title>
@elseif($action=='engineer-list')
<title>{{websiteTitle('Company Engineer List')}}</title>
@else
<title>{{websiteTitle('Company View Export')}}</title>
@endif
@endsection 
@push('css')

<style type="text/css">
    .nav-tabs .nav-link {
        font-size: 16px;
        font-weight: bold;
    }
    .nav-tabs .nav-link.active {
        color: #495057;
        background-color: #e9ecef;
        border-color: #dee2e6 #dee2e6 #fff;
    }
    
    
    .status span {
        background: #f2f2f2;
        padding: 5px 15px;
        display: inline-block;
        margin-bottom: 5px;
        border-radius: 5px;
        color: #4c4a4a;
        font-weight: bold;
        border: 1px solid #c6c6c6;
    }
    
    .emiTable tr th{
        padding:2px 5px;
    }
    .emiTable tr td{
        padding:2px 5px;
    }
    
    @media only screen and (max-width: 678px) {
        .nav-tabs .nav-item {
            width: 50%;
            text-align: center;
            border: 1px solid #e7e7e7;
        }
    }
    
    
</style>
@endpush 
@section('contents')

<div class="content-header row">
    <div class="content-header-left col-md-8 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="breadcrumb-wrapper col-12">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">Dashboard </a></li>
                    <li class="breadcrumb-item active">
                        @if($action=='pm-list')
                        Company PM List
                        @elseif($action=='partner-list')
                        Partner List
                        @elseif($action=='keyperson-list')
                        Key person List
                        @elseif($action=='operator-list')
                        Operator List
                        @elseif($action=='engineer-list')
                        Engineer List
                        @else
                        
                        @endif
                    </li>
                </ol>
            </div>
        </div>
    </div>
    <div class="content-header-right col-md-4 col-12 mb-md-0 mb-2">
        <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
            <a class="btn btn-outline-primary" href="{{route('admin.companies')}}">BACK</a>
            <a class="btn btn-outline-primary" href="{{route('admin.companiesAction',$action)}}">
                <i class='bx bx-loader-circle' ></i>
            </a>
        </div>
    </div>
</div>


<section class="flex-grow-1">
@include(adminTheme().'alerts')
    
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-3">
                    <div class="card-header" style="border-bottom: 1px solid #e3ebf3;">
                        <h4 class="card-title">
                            @if($action=='pm-list')
                            Company PM List
                            @elseif($action=='partner-list')
                            Partner List
                            @elseif($action=='keyperson-list')
                            Key person List
                            @elseif($action=='operator-list')
                            Operator List
                            @elseif($action=='engineer-list')
                            Engineer List
                            @else
                            
                            @endif
                        </h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table dataex-html5-export">
                                    <thead>
                                        <tr>
                                            <th style="min-width: 100px;width: 100px;padding-right:0;"> SL </th>
                                            <th style="min-width: 200px;">Customer/Company Name</th>
                                            @if($action=='pm-list' || $action=='engineer-list' || $action=='manager-list')
                                            <th style="min-width: 200px;">Name</th>
                                            <th style="min-width: 150px;">Designation</th>
                                            <th style="min-width: 150px;">Details</th>
                                            @elseif($action=='keyperson-list')
                                            <th style="min-width: 200px;">Name</th>
                                            <th style="min-width: 150px;">Designation</th>
                                            <th style="min-width: 150px;">Mobile No</th>
                                            <th style="min-width: 150px;">Whatsapps number</th>
                                            <th style="min-width: 150px;">E-mail</th>
                                            @elseif($action=='operator-list' || $action=='partner-list')
                                            <th style="min-width: 200px;">Name</th>
                                            <th style="min-width: 150px;">Mobile</th>
                                            <th style="min-width: 150px;">Address</th>
                                            <th style="min-width: 150px;">Details</th>
                                            @else
                                            
                                            @endif
                                            

                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($companies as $i=>$company)
                                            <tr>
                                                <td style="position: relative;">
                                                    <span style="margin:0 5px;">{{$i+1}}</span>
                                                </td>
                                                @if($action=='operator-list' || $action=='partner-list')
                                                <td>
                                                    {{$company->company?$company->company->owner_name:''}}
                                                    @if(!empty($company->company->factory_name))
                                                        - {{ $company->company->factory_name }}
                                                    @endif
                                                </td>
                                                @else
                                                <td>{{$company->factory_name?:$company->owner_name}}</td>
                                                @endif
                                                @if($action=='pm-list')
                                                <td>{{$company->pm_name}}</td>
                                                <td>{{$company->pm_designation}}</td>
                                                <td>{{$company->pm_details}}</td>
                                                @elseif($action=='keyperson-list')
                                                <td>{{$company->key_parson_name}}</td>
                                                <td>{{$company->key_parson_designation}}</td>
                                                <td>{{$company->key_parson_mobile}}</td>
                                                <td>{{$company->key_parson_whatsapp_mobile}}</td>
                                                <td>{{$company->key_parson_email}}</td>
                                                @elseif($action=='operator-list' || $action=='partner-list')
                                                <td>{{$company->name}}</td>
                                                <td>{{$company->mobile}} {{$company->mobile2?' - '.$company->mobile2:''}}</td>
                                                <td>{{$company->fullAddress()}}</td>
                                                <td>{{$company->description}}</td>
                                                @elseif($action=='manager-list')
                                                <td>{{$company->manager_name}}</td>
                                                <td>{{$company->manager_designation}}</td>
                                                <td>{{$company->manager_details}}</td>
                                                @elseif($action=='engineer-list')
                                                <td>{{$company->engineer_name}}</td>
                                                <td>{{$company->engineer_designation}}</td>
                                                <td>{{$company->engineer_details}}</td>
                                                @else
                                                
                                                @endif
                                                
                                                
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
                
            </div>
        </div>
    
</section>
<!-- Basic Inputs end -->


@endsection 
@push('js')
<!-- BEGIN: Page Vendor JS-->
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/datatable/datatables.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/datatable/dataTables.buttons.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/datatable/buttons.bootstrap4.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/jszip.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/pdfmake.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/vfs_fonts.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/buttons.html5.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/buttons.print.min.js')}}"></script>
<script src="{{asset(assetLinkAdmin().'/app-assets/vendors/js/tables/buttons.colVis.min.js')}}"></script>

<!-- END: Page Vendor JS-->

<script type="text/javascript">
    $(document).ready(function () {
            $(".dataex-html5-export").DataTable({ dom: "Bfrtip", buttons: ["copyHtml5", "excelHtml5", "csvHtml5", "pdfHtml5", "print"],pageLength: 100 }),
        
            $(".dataex-visibility-print").DataTable({ dom: "Bfrtip", buttons: ["print"] }),
            $(".dataex-html5-separated").DataTable({ dom: "Bfrtip", buttons: ["copyHtml5", { extend: "csvHtml5", fieldSeparator: "\t", extension: ".tsv" }] }),
            $(".dataex-html5-selectors").DataTable({
                dom: "Bfrtip",
                buttons: [{ extend: "copyHtml5", exportOptions: { columns: [0, ":visible"] } }, { extend: "excelHtml5", exportOptions: { columns: ":visible" } }, { extend: "pdfHtml5", exportOptions: { columns: [0, 1, 2, 5] } }, "colvis"],
            }),
            $(".dataex-html5-background").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        extend: "excelHtml5",
                        customize: function (t) {
                            var e = t.xl.worksheets["sheet1.xml"];
                            $('row c[r^="F"]', e).each(function () {
                                1 * $("is t", this).text().replace(/[^\d]/g, "") >= 5e5 && $(this).attr("s", "20");
                            });
                        },
                    },
                ],
            }),
            $(".dataex-html5-message").DataTable({ dom: "Bfrtip", buttons: [{ extend: "pdfHtml5", message: "PDF created by PDFMake with Buttons for DataTables." }] }),
            $(".dataex-html5-orientation").DataTable({ dom: "Bfrtip", buttons: [{ extend: "pdfHtml5", orientation: "landscape", pageSize: "LEGAL" }] }),
            $(".dataex-html5-image").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        extend: "pdfHtml5",
                        customize: function (t) {
                            t.content.splice(1, 0, {
                                margin: [0, 0, 0, 12],
                                alignment: "center",
                                image:
                                    "data:image/png;base64,",
                            });
                        },
                    },
                ],
            }),
            $(".dataex-html5-window").DataTable({ dom: "Bfrtip", buttons: [{ extend: "pdfHtml5", download: "open" }] }),
            $(".dataex-html5-json").DataTable({
                dom: "Bfrtip",
                buttons: [
                    {
                        text: "JSON",
                        action: function (t, e, a, A) {
                            var o = e.buttons.exportData();
                            $.fn.dataTable.fileSave(new Blob([JSON.stringify(o)]), "Export.json");
                        },
                    },
                ],
            });
    });
</script>
@endpush
