...
@section('title')
    <title>{{ websiteTitle('Edit Probation') }}</title>
@endsection
@section('contents')
...
<div class="flex-grow-1">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.probations.update', $probation->id) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="user_id" class="form-control" required>
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ $probation->user_id == $user->id ? 'selected' : '' }}>{{ $user->name }} [{{ $user->employee_id }}]</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="months" class="form-control" min="1" value="{{ $probation->months }}" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
