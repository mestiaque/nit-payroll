...
<title>{{websiteTitle('Quotations List')}}</title>
@endsection @push('css')
...
<div class="card mb-30">
    <div class="card-header d-flex justify-content-between align-items-center">
         ...
         <div class="dropdown">
            @isset(json_decode(Auth::user()->permission->permission, true)['quotation']['add'])
             <a href="{{route('admin.quotationsAction','create')}}" class="btn-custom primary" style="padding:5px 15px;">
                 <i class="bx bx-plus"></i> Add Invoice
             </a>
            @endisset
            
             <a href="{{route('admin.quotations')}}" class="btn-custom yellow">
    ...
    <div class="card-body">
        ...
        <form action="{{route('admin.quotations')}}">
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    <ul class="statuslist p-0">
                        <li><a href="{{route('admin.quotations')}}">All ({{$totals->total}})</a></li>
                        <li><a href="{{route('admin.quotations',['status'=>'pending'])}}">Pending ({{$totals->pending}})</a></li>
                        <li><a href="{{route('admin.quotations',['status'=>'confirmed'])}}">Confirmed ({{$totals->confirmed}})</a></li>
                        <li><a href="{{route('admin.quotations',['status'=>'completed'])}}">Completed ({{$totals->completed}})</a></li>
                        <li><a href="{{route('admin.quotations',['status'=>'cancelled'])}}">Cancelled ({{$totals->cancelled}})</a></li>
                        <li><a href="{{route('admin.quotations',['status'=>'trash'])}}">Trash ({{$totals->trash}})</a></li>
                    </ul>
                </div>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if($invoice->hasSubInvoices->count() > 0)({{$invoice->hasSubInvoices->count()}}) @endif
                            @if($invoice->hasLcOrder)
                            @if($invoice->hasLcOrder->order)
                            <br>
                            <b>LC:</b> {{$invoice->hasLcOrder->order->invoice}}
                            @endif
                            @endif
                            </td>
                            <td>@if($invoice->company) {{$invoice->company->factory_name}} @endif</td>
                            <td>{{$invoice->items()->count()}} Items</td>
                            <td>BDT {{$invoice->grand_total}}</td>
                            <td>{{$invoice->created_at->format('d.m.Y')}}</td>
                            <td>
                                @if($invoice->hasLcOrders->count() > 0)
                                @isset(json_decode(Auth::user()->permission->permission, true)['quotation']['view'])
                                <a href="{{route('admin.quotationsAction',['view',$invoice->id])}}" class="btn-custom success"><i class="bx bx-show"></i></a>
                                @endisset
                                @else
                                @isset(json_decode(Auth::user()->permission->permission, true)['quotation']['add'])
                                <a href="{{route('admin.quotationsAction',['edit',$invoice->id])}}" class="btn-custom"><i class="bx bx-edit"></i></a>
                                @endisset
                                @endif
                                @isset(json_decode(Auth::user()->permission->permission, true)['quotation']['delete'])
                                <a href="{{route('admin.quotationsAction',['delete',$invoice->id])}}" onclick="return confirm('Are You Want To Delete?')" class="btn-custom danger"><i class="bx bx-trash"></i></a>
                                @endisset
                            </td>
                        </tr>
                        @endforeach
